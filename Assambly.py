...
names = [f"Sparse_01_{nnodos}x{ncores}.txt",f"Sparse_015_{nnodos}x{ncores}.txt",f"Sparse_02_{nnodos}x{ncores}.txt",f"Dense_{nnodos}x{ncores}.txt"]
files = [open(name,"w") for name in names]
if rank == 0:
    print(f"Trabajando con {nnodos} nodos y {ncores} nucleos")

for N in list1:
    Bandwitch = [int(0.001*N),int(0.0015*N),int(0.002*N)]
    print (f"N = {N}")
    print (f"Rank = {rank}\n")
