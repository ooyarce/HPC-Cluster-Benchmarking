...
names = [f"Sparse_90_{nnodos}x{ncores}.txt",f"Sparse_95_{nnodos}x{ncores}.txt",f"Sparse_98_{nnodos}x{ncores}.txt",f"Dense_MPI{nnodos}x{ncores}.txt"]
...
for N in list1:
    Bandwitch = [int(0.9*N),int(0.95*N),int(0.98*N)]
    print (f"N = {N}")
    print (f"Rank = {rank}\n")
...
if rank == 0:
    ...
    plt.title(f"Solving time {nnodos}nodes-{ncores}cores")
    plt.tight_layout()
    plt.show()
    plt.savefig(f"Solving{nnodos}x{ncores}")